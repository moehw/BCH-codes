...
class Matrix:
    def __init__(self, rows=1, cols=1, data=None, fill=None):
        if data is None:
            data = []
        self.rows = rows
        self.cols = cols
    ...
    def distance(self, matrix):
        rows = min(self.rows, matrix.rows)
        cols = min(self.cols, matrix.cols)

        dist = []
        for i in range(rows):
            for j in range(cols):
                if self.get(i,j) != matrix.get(i,j):
                    dist.append([i,j])
    ...
    def swap_cols(self, c1, c2):
        col1_backup = []
        for r in xrange(self.rows):
            col1_backup.append(self.get(r, c1))
            self.set(r, c1, self.get(r, c2))
            self.set(r, c2, col1_backup.pop())
    
    def copy_col(self, c1, matrix, c2):
        rows = min(self.rows, matrix.rows)
        for r in xrange(rows):
            self.set(r, c1, matrix.get(r, c2))
